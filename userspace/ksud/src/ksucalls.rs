...
pub fn set_app_profile_allow_su(key: &str, uid: u32, allow: bool) -> Result<()> {
    if key.len() >= ksu_uapi::KSU_MAX_PACKAGE_NAME as usize {
        anyhow::bail!("profile key too long: {key}");
    }

    let mut profile = unsafe { std::mem::zeroed::<ksu_uapi::app_profile>() };
    profile.version = ksu_uapi::KSU_APP_PROFILE_VER;
    for (idx, byte) in key.as_bytes().iter().enumerate() {
        profile.key[idx] = *byte as c_char;
    }
    profile.curr_uid = i32::try_from(uid).context("uid overflows i32")?;
    profile.allow_su = allow;
    unsafe {
        let rp_config = &mut profile.__bindgen_anon_1.rp_config;
        rp_config.use_default = true;
        let domain = b"u:r:ksu:s0";
        for (idx, byte) in domain.iter().enumerate() {
            rp_config.profile.selinux_domain[idx] = *byte as c_char;
        }
    }

    let mut cmd = ksu_uapi::ksu_set_app_profile_cmd { profile };
